...
export class EditNewFicheAnimalComponent implements OnInit {
  ...
  constructor(private ficheAnimalService: FicheAnimalService, private route: ActivatedRoute, private router: Router) {
  ...
  public addFicheAnimal(ficheAnimal: FicheAnimal): void {
    this.ficheAnimalService.createFicheAnimal(this.ficheAnimal_in_progress).subscribe(
      (response) => {
        this.router.navigateByUrl('/compteAdherent');
      }
    );
